...
run_deploy_check() {
  step "deploy-check" "Polygon Mumbai デプロイ準備の dry-check"
  bash scripts/deploy_mumbai.sh --dry-check 2>&1 | sed 's/^/    /' || fail "deploy-check NG"
  ok "deploy-check"
}
...
" > /tmp/loadsrv.log 2>&1 ) &
  ...
  wait 2>/dev/null
  rm -f /tmp/loadtest.db /tmp/loadtest.db-wal /tmp/loadtest.db-shm
  ok "load"
}
